...
def _clear_runs(p: Paragraph):
    """
    WARNING: This destroys formatting! Use _safe_set_paragraph_text() instead when possible.
    Only use this when you need to completely rebuild a paragraph with new runs.
    """
    for r in list(p.runs):
        r._element.getparent().remove(r._element)


def _safe_set_paragraph_text(p: Paragraph, new_text: str):
    """
    Set paragraph text while PRESERVING formatting from the first run.
    
    Use this instead of: _clear_runs(p); p.add_run(new_text)
    
    This preserves the font, size, bold, and other formatting from the first run.
    """
    if p.runs:
        ...
        # No runs exist, must add one (formatting will be default)
        p.add_run(new_text)
...
def _replace_in_paragraph_run_aware(p: Paragraph, mapping: Dict[str, str]) -> bool:
    ...
    Key insight: Instead of clearing runs and adding new ones (which destroys formatting),
    we put all text in the first run and clear the rest. This preserves the first run's
    formatting (font, size, bold, etc.) for the entire paragraph.
    """
    # Get the full paragraph text (concatenated from all runs)
    full_text = p.text
    if not full_text:
        return False
    ...
    # Put all text in first run, clear others (preserves first run's formatting)
    if p.runs:
        p.runs[0].text = new_text
        for run in p.runs[1:]:
            run.text = ""
...
def _fix_questioned_costs_grammar(doc):
    ...
    for p in _iter_all_paragraphs_full(doc):
        ...
        if "No questioned cost is included" in text:
            ...
            if new_text != text:
                _safe_set_paragraph_text(p, new_text)
    ...
    def _strip_leftovers_in_container(container):
        for p in _iter_all_paragraphs_in_container(container):
            ...
            if "[" in new_t and "]" in new_t:
                new_t = re.sub(r"\[[^\]]+\]", "", new_t)
            if new_t != t:
                _safe_set_paragraph_text(p, new_t)
...
def _rewrite_paragraph(p, text):
    """Rewrite paragraph text while preserving formatting."""
    _safe_set_paragraph_text(p, text)
...
def _strip_leading_token_artifacts(doc):
    ...
    for p in _iter_all_paragraphs(doc):
        ...
        new = pat.sub("", t)
        if new != t:
            _safe_set_paragraph_text(p, new)

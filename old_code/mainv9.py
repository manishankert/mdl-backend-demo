...
def upload_and_sas(container: str, blob_name: str, data: bytes, ttl_minutes: int = 120) -> str:
    ...
    if proto:
        sas_params['protocol'] = proto
    sas = generate_blob_sas(**sas_params)
...
def build_docx_from_template(model: Dict[str, Any], *, template_path: str) -> bytes:
    ...
    # Optional tidy: strip any *remaining* token patterns like ${...} or [...]
    ...
    if not anchor:
        # Fallback: append tables at end
        anchor = doc.add_paragraph("[[PROGRAM_TABLES]]")
    programs = model.get("programs", []) or []
    _insert_program_tables_at_anchor(doc, anchor, programs)
    _delete_paragraph(anchor)

    _inject_email_everywhere(doc, "ORP_SingleAudits@treasury.gov")


    # Post-process: pluralization and font normalization
    try:
...
def _inject_email_everywhere(doc, email: str):
    import re as _re
    def norm(s): 
        return (s or "").replace("\u00A0"," ").replace("\xa0"," ")
    patterns = [
        _re.compile(r"(via\s+email\s+at)(\s*)(?!\S)", _re.IGNORECASE),
        _re.compile(r"(email\s+us\s+at)(\s*)(?!\S)", _re.IGNORECASE),
    ]
    def fix_para(p):
        ...
        new = nt
        for pat in patterns:
            new = pat.sub(lambda m: m.group(1) + " " + email, new)
        if new != nt:

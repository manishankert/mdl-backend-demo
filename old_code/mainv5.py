...
def _basic_html_to_docx(doc: Document, html_str: str):
    ...
    for element in body.children:
        ...
        if txt:
            doc.add_paragraph(txt)
def _load_bucket_rules(xlsx_path: Optional[str]):
    rules = []  # list of dicts {ctype, label, keywords}
    if not xlsx_path or not os.path.exists(xlsx_path): return rules
    wb = openpyxl.load_workbook(xlsx_path, data_only=True)
    for ws in wb.worksheets:
        hdrs = [c.value.strip() if isinstance(c.value, str) else c.value for c in ws[1]]
        try:
            i_ct = hdrs.index("Compliance Type")
            i_lb = hdrs.index("Bucket Label")
        except ValueError:
            continue
        i_kw = hdrs.index("Keywords") if "Keywords" in hdrs else None
        for row in ws.iter_rows(min_row=2, values_only=True):
            ct = (row[i_ct] or "").strip()
            lb = (row[i_lb] or "").strip()
            kw = (row[i_kw] or "").strip() if i_kw is not None else ""
            if ct and lb:
                rules.append({"ctype": ct.upper(), "label": lb, "keywords": [k.strip().lower() for k in kw.split(",") if k.strip()]})
        break
    return rules

def bucketize(summary: str, compliance_type: str, rules) -> Optional[str]:
    s = (summary or "").lower()
    ct = (compliance_type or "").upper().strip()
    # exact ctype match with keyword boost
    candidates = [r for r in rules if r["ctype"] == ct] or rules
    best = None
    score = -1
    for r in candidates:
        sc = 0
...
def render_mdl_html(model: Dict[str, Any]) -> str:
    ...
    def _render_program_table(p: Dict[str, Any]) -> str:
        ...
        for f in p.get("findings", []):
            rows_html.append(f"""
              <tr>
                ...
                <td>{html.escape(f.get('bucket_label',''))}</td>
        ...
        if not rows_html:
            rows_html.append("""
              <tr><td colspan="7"><em>No MDL-relevant findings identified for this program.</em></td></tr>
            """)
        table = f"""
    ...
    chunks.append("""
      ...
      <p>For questions regarding the audit finding(s), please contact Treasury. Thank you.</p>
...
def build_mdl_model_from_fac(
    ...
    **_
    ...
    """
    Builds the MDL model. Enhancements vs original:
      - Canonicalizes Assistance Listing and Program Name using an Excel reference
        (e.g., '21.027' -> 'Coronavirus State and Local Fiscal Recovery Funds (SLFRF)').
      - If ALN is missing from FAC, tries to infer via the Excel (by program name).
      - Applies treasury_listings filter AFTER canonicalization.
    """
    # ---------- helpers ----------
    ...
    def _load_aln_mapping(xlsx_path: Optional[str]):
        ...
        for ws in wb.worksheets:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                name_to_aln[prog_name.lower()] = aln
            break
        return aln_to_label, name_to_aln

    def _canonicalize_group_fields(group: Dict[str, Any], aln_to_label: Dict[str, str], name_to_aln: Dict[str, str]):
        cur_aln = str(group.get("assistance_listing") or "").strip()
        ...
        if cur_aln and cur_aln in aln_to_label:
            ...
            return
        if (not cur_aln) or (cur_aln == "Unknown"):
            ...
            if guess_aln:
                ...
                return
        group["assistance_listing"] = cur_aln or "Unknown"
        group["program_name"] = _title_with_acronyms(cur_name or "Unknown Program")

    # ---------- load mapping ----------
    aln_to_label, name_to_aln = _load_aln_mapping(aln_reference_xlsx)
    bucket_rules = _load_bucket_rules(aln_reference_xlsx)
    ...
    for r in base_refs:
        ...
        k = _norm_ref(r)
        if k not in seen:
            seen.add(k); norm_refs.append((r, k))
    ...
    # ---------- canonicalize after grouping ----------
    for grp in programs_map.values():
        _canonicalize_group_fields(grp, aln_to_label, name_to_aln)

    # ---------- filter after canonicalization ----------
...
def build_docx_from_template(model: Dict[str, Any], *, template_path: str) -> bytes:
    ...
    _remove_watermarks(doc)  # remove watermarks like DRAFT early
    ...
    # (moved) Optional tidy: will run AFTER anchor insertion
...
# Final tidy (run after anchor handling)
def _strip_leftovers_in_container(container):
    for p in _iter_all_paragraphs_in_container(container):
        t = _para_text(p)
        if not t:
        ...
        if "${" in new_t:
            new_t = re.sub(r"\$\{[^}]+\}", "", new_t)
        if "[" in new_t and "]" in new_t:
            # keep the anchor already consumed; this will only hit other bracketed tokens
            new_t = re.sub(r"\[[^\]]+\]", "", new_t)
        if new_t != t:
            _clear_runs(p)
            p.add_run(new_t)
    ...
    # Ensure email appears inline where expected
    _inject_email_if_missing(doc, "ORP_SingleAudits@treasury.gov")

    # Apply pluralization for tokens like Finding(s), CAP(s), Program(s)
    _apply_pluralization(doc, counts={
        "Finding": sum(len(p.get("findings", [])) for p in (model.get("programs") or [])),
        "CAP": sum(1 for p in (model.get("programs") or []) for f in p.get("findings", []) if f.get("cap_text")),
        "Program": len(model.get("programs") or []),
        "award": len(model.get("programs") or []),
    })

    # Enforce Times New Roman 12pt across the document
    _force_font(doc, family="Times New Roman", size_pt=12)
...
def _remove_watermarks(doc):
    """
    Removes WordArt/VML watermark shapes from docx parts (common 'DRAFT' watermark).
    """
    try:
        for part in doc.part.package.parts:
            ...
            if root is None or not hasattr(root, "xpath"):
                continue
            ns = root.nsmap
            for pict in list(root.xpath(".//w:pict", namespaces=ns)):
                kill = False
                for shp in pict.xpath(".//v:shape", namespaces=ns):
                    ...
                    if "PowerPlusWaterMarkObject" in sid or "WaterMark" in sid:
                        kill = True; break
                    for tx in shp.xpath(".//v:textpath[@string]", namespaces=ns):
...
def _inject_email_if_missing(doc, email):
    for p in doc.paragraphs:
        t = "".join(r.text for r in p.runs)
        if "via email at" in t and (email not in t):
            new = t.replace("via email at", f"via email at {email}")
            _clear_runs(p); p.add_run(new)

def _apply_pluralization(doc, counts: Dict[str, int]):
    import re as _re
    pairs = []
    for base, n in counts.items():
        singular = base
        plural = base + ("s" if not base.endswith("s") else "")
        pairs.append((singular, plural, n))

    def fix_text(text: str) -> str:
        out = text
        for sing, plur, n in pairs:
            pattern = _re.compile(rf'\\b{_re.escape(sing)}\\(s\\)\\b', _re.IGNORECASE)
            def repl(m):
                token = m.group(0)
                use = sing if n == 1 else plur
                if token.isupper(): return use.upper()
                if token[0].isupper(): return use.capitalize()
                return use.lower()
    ...
    walk(doc)
    for sec in doc.sections:
        walk(sec.header); walk(sec.footer)

from docx.shared import Pt
def _force_font(doc, family="Times New Roman", size_pt=12):
    try:
    ...
    paint(doc)
    for sec in doc.sections:
        paint(sec.header); paint(sec.footer)
...
def build_mdl_docx_by_report_templated(req: BuildByReportTemplated):
    try:
        ...
        mdl_model = build_mdl_model_from_fac(
            ...
            fac_caps=fac_caps,
            federal_awards=federal_awards,
            period_end_text=req.fy_end_text,
            address_lines=[req.street_address, f"{req.city}, {req.state} {req.zip_code}"],
            attention_line=f"ATTN: {req.poc_name}, {req.poc_title}",
            only_flagged=req.only_flagged,
            max_refs=req.max_refs,
            include_no_qc_line=True,
            include_no_cap_line=True,
            treasury_listings=getattr(req, "treasury_listings", None),
            aln_reference_xlsx=getattr(req, "aln_reference_xlsx", None),
        )
...
class BuildAuto(BaseModel):
    ...
    include_awards: bool = True
    treasury_listings: Optional[List[str]] = None
    # optional header overrides (all optional)
    recipient_name: Optional[str] = None
    fy_end_text: Optional[str] = None
    auditor_name: Optional[str] = None
    street_address: Optional[str] = None
